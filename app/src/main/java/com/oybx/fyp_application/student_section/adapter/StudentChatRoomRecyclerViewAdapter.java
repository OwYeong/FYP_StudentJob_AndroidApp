...
public class StudentChatRoomRecyclerViewAdapter extends RecyclerView.Adapter<StudentChatRoomRecyclerViewAdapter.MyViewHolder> {
    private Context myContext;
    private Fragment parentFragment;
    private ArrayList<ChatRoomInfo> chatRoomInfoArrayList = new ArrayList<>();//Array list For storing Employer Post info


    public StudentChatRoomRecyclerViewAdapter(Context myContext, Fragment parentFragment, ArrayList<ChatRoomInfo> chatRoomInfoArrayList) {
                ...
                currentActivity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (currentLayoutName.equals("StudentMessagepage")) {
                            ...
                            FragmentStudentChatroompage.setSelectedChatRoomInfo(currentChatRoomInfo);
    ...
    public int getItemCount() {
        ...
        if(chatRoomInfoArrayList.size() == 0){
            ((FragmentStudentMessagepage)parentFragment).displayNoMessageSection(true);
        }else {
            ((FragmentStudentMessagepage)parentFragment).displayNoMessageSection(false);
        }
